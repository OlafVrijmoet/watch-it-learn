...
Reading about attention and backprop is one thing; watching them is another. I was curious to actually see how a transformer learns, so I built a tool where you compose the architecture yourself, press train, then replay the whole run and watch the residual stream light up, attention sharpen, and the gradient signal flow back. Every number on screen is real and recomputed from the model, not a canned animation.
